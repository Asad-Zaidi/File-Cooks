...
import { AUDIO_FORMAT_SPECS, AUDIO_CONFIG } from '../../services/audioService';
import { formatBytes } from '../../utils/filenameUtils';

export default function AudioQueueHeader({
  itemsCount = 0,
  totalSizeBytes = 0,
  globalTargetFormat = 'mp3',
  ...
  return (
    <div className="bg-white border border-gray-200/90 rounded-3xl p-5 sm:p-6 shadow-sm mb-6 transition-all">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        ...
        {/* Center/Right: Actions & Target Format Selector */}
        <div className="flex flex-wrap items-center gap-2.5 sm:gap-3">
          {/* Global Target Format Picker */}
          <div className="flex items-center gap-2 bg-gray-50 border border-gray-200/80 rounded-2xl p-1.5 pl-3">
            <span className="text-xs font-bold text-gray-500 whitespace-nowrap">Convert all to:</span>
            <select
            ...
            >
              {AUDIO_CONFIG.supportedOutputFormats.map((fmtKey) => {
                const spec = AUDIO_FORMAT_SPECS[fmtKey];
                return (
                  <option key={fmtKey} value={fmtKey}>
                    {spec?.label || fmtKey.toUpperCase()} {spec?.lossless ? '(Lossless)' : ''}
                  </option>
                );
              })}
            </select>
          ...
          <button
            ...
            title="Audio Quality & Encoder Settings"
          >
            <FaSlidersH size={13} />
          ...
          {/* Download All ZIP Button (if any item completed) */}
          {hasCompleted && (
            <button
              type="button"
          ...
          >
            {isProcessingBatch ? (
              ...
              <>
                <FaPlay size={11} />
                <span>Convert All ({itemsCount - completedCount})</span>
              </>
            )}
          </button>
        </div>
